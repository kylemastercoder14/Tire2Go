...
export const columns: ColumnDef<Faqs>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2 ml-2.5">
          <div>
            <span className="font-semibold">{raw.question}</span>
            <div
              title={raw.id}
              ...
              {copied ? (
                ...
                <CopyIcon
                  onClick={() => {
                    ...
                    toast.success("FAQs ID copied to clipboard");
                    setCopied(true);
    ...
    filterFn: (row, columnId, filterValue) => {
      const name = (row.original.question ?? "").toLowerCase();
      const answer = (row.original.answer ?? "").toLowerCase();
      const id = (row.original.id ?? "").toLowerCase();
      const search = filterValue.toLowerCase();

      return (
        name.includes(search) || id.includes(search) || answer.includes(search)
      );
    },
    enableSorting: false,
    enableHiding: false,
